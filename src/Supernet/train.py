...
def get_args():
    ...
    parser.add_argument('--eval-resume', type=str, default='./models/checkpoint-latest.pth.tar', help='path for eval model')
...
def main():
    ...
    meta_optimizer = torch.optim.SGD(get_parameters(meta_model),
                                lr=args.learning_rate,
                                momentum=args.momentum,
                                weight_decay=args.weight_decay)
    meta_scheduler = torch.optim.lr_scheduler.LambdaLR(meta_optimizer,
                    lambda step : (1.0-step/args.total_iters) if step <= args.total_iters else 0, last_epoch=-1)
    ...
    args.meta_optimizer = meta_optimizer
    args.loss_function = loss_function
    args.scheduler = scheduler
    args.meta_scheduler = meta_scheduler
    ...
    while all_iters < args.total_iters:
        all_iters = train(model, meta_model, device, args, val_interval=args.val_interval, bn_process=False, all_iters=all_iters)
...
def train(model, meta_model, device, args, *, val_interval, bn_process=False, all_iters=None):
